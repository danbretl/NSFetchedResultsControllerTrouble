...
- (void) swipedOnHeader:(UISwipeGestureRecognizer *)swipeGestureRecognizer;
...
- (void)viewDidLoad {
    ...
    self.swipeRightHeaderGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipedOnHeader:)];
    ...
    self.swipeLeftHeaderGestureRecognizer = [[UISwipeGestureRecognizer alloc] initWithTarget:self action:@selector(swipedOnHeader:)];
...
- (void) updateViewsForCurrentFocus {
    ...
    self.photosScrollView.contentSize = CGSizeMake([[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects] * self.photosScrollView.frame.size.width, self.photosScrollView.frame.size.height);
...
- (void)imageDownloader:(SDWebImageDownloader *)downloader didFinishWithImage:(UIImage *)image {
    ...
    NSString * photoServerID = [downloader.userInfo objectForKey:@"serverID"];
    NSUInteger photoViewIndexOfPhotoServerID = [self.photoViewsPhotoServerIDs indexOfObject:photoServerID];
    ...
    NSLog(@"imageDownloader didFinishWithImage forPhotoWithServerID %@", photoServerID);
}
...
- (Photo *) photoAtIndex:(int)index {
    Photo * photo = nil;
    if (index >= 0 && index < [[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects]) {
...
//    NSLog(@"self.fetchedResultsControllerForCurrentFocus didChangeObject:%@ atIndexPath:%d-%d forChangeType:%d newIndexPath:%d-%d", anObject, indexPath.section, indexPath.row, type, newIndexPath.section, newIndexPath.row);
    ...
    //                NSLog(@"Going to update photoView %@ at photoViewIndex=%d withPhotooAtIndex:%d", [self.photoViews objectAtIndex:photoViewIndex], photoViewIndex, indexPath.row);
                ...
                if (indexPath.row == self.photoCenterIndex) {
                    ...
                    if ([[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects] == 0) {
                        ...
                        [UIScrollView animateWithDuration:0.25 animations:^{ // Note the receiver Class UIScrollView here. New trick I hadn't known before!
                            self.photoViewCenter.alpha = 0.0;
                            if (self.photoCenterIndex < [[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects]) {
                            ...
                            [self setPhotoCenterIndex:MIN(self.photoCenterIndex, [[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects] - 1) forcePhotoViewsUpdate:YES];
...
- (void)controllerDidChangeContent:(NSFetchedResultsController *)controller {
    if (controller == self.fetchedResultsControllerForCurrentFocus && 
        ...
        if (self.refreshAllInProgress) {
            NSLog(@"self.refreshAllInProgress");
            ...
            if ([[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects] == 0) {
                ...
                if (oldPhotoCenterHidden) {
                    ...
                    newPhotoCenterIndex = MIN(MAX(0, newPhotoCenterIndex), (int)(self.photosScrollView.contentSize.width - 1) / [[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects]);
                } else {
...
- (void)photoViewSelected:(PhotoView *)photoView withPhoto:(Photo *)photo {
    ...
    if (!self.finishing) {
        ...
        [UIView animateWithDuration:0.25 delay:0.0 options:UIViewAnimationCurveEaseIn animations:^{
            ...
            NSLog(@"About to [self.delegate photosStripViewController:self requestedReplacementWithPhotosStripViewController:oppositeFocusStripViewController];");
...
- (void)swipedVertically:(UISwipeGestureRecognizer *)swipeGestureRecognizer {
    ...
    if (self.focus == FeelingFocus &&
        (!(indexPath.row + indexAdjustment < 0 || 
           indexPath.row + indexAdjustment >= [[[self.fetchedResultsControllerFeelings sections] objectAtIndex:0] numberOfObjects]))) {
        ...
        [feelingViewController setFocusToFeeling:nextFeeling photo:[nextFeeling.mostRecentPhotosVisible objectAtIndex:0]];
...
//                feelingViewController.backgroundView.alpha = 1.0;
            ...
            [feelingViewController.cameraButtonView setButtonPromptVisible:YES];
            feelingViewController.swipingInVertically = YES;
...
- (void) swipedOnHeader:(UISwipeGestureRecognizer *)swipeGestureRecognizer {
    ...
    BOOL directionLeft = swipeGestureRecognizer.direction == UISwipeGestureRecognizerDirectionLeft;
    ...
    if (acceptSwipe) {
        ...
        CGFloat positionXAdjustment = (directionLeft ? -1 : 1) * (self.headerButton.contentEdgeInsets.left - screenBufferLeft);
        animation.toValue = [NSValue valueWithCGPoint:CGPointMake(self.headerButton.layer.position.x + positionXAdjustment, self.headerButton.layer.position.y)];
        [self.headerButton.layer addAnimation:animation forKey:nil];
        
        CGPoint contentOffsetAdjusted = CGPointZero;
        if (!directionLeft) {
...
//    self.refreshAllInProgress = NO; // This is taken care of in didChangeContent. No... Trying to take care of this at the end of this method. // Nope, putting it back in didChangeContent
    NSLog(@"PhotosStripViewController got %d photos", photosFromWeb.count);
...
//    if ([[[self.fetchedResultsControllerForCurrentFocus sections] objectAtIndex:0] numberOfObjects] == 0) {
//        [self.delegate photosStripViewControllerFinished:self withNoMorePhotos:YES];
//    }
}
